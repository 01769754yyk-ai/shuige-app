...
export default function Profile(){
  ...
  useEffect(()=>{
    ...
    const t=get(TASK_KEY); if(t&&t.length) setTasks(t);
  ...
  const settings=[['🎨','主题与角色'],['🧩','首页布局'],['🔔','提醒与专注'],['🔒','数据与隐私']];

  const save=()=>{ if(draft.trim()){setName(draft.trim());localStorage.setItem(NAME_KEY,draft.trim())} setEditing(false); };
  const exportAll=()=>{ const data={name,countdown:counts}; const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='水哥清单-数据备份.json'; a.click(); };
  ...
  return (
    <div style={{minHeight:'100vh',padding:'16px 14px 80px',background:'#F4F7F0',color:'#3A4437',fontSize:15}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:14,background:'#fff',border:'1px solid #E5ECDE',borderRadius:20,padding:16,marginTop:12}}>
        ...
        <button onTouchStart={()=>{setEditing(true);setDraft(name)}} onClick={()=>{setEditing(true);setDraft(name)}} style={{background:'#F4F7F0',border:'1px solid #E5ECDE',borderRadius:11,padding:'6px 12px',fontSize:13,color:'#3A4437',touchAction:'manipulation'}}>改昵称</button>
      </div>
      ...
      <div style={{marginTop:18}}>
        <div style={{fontSize:13,color:'#A99585',marginBottom:8}}>设置</div>
        {settings.map(([icon,label])=>(
          <button key={label} onClick={()=>alert(icon+' '+label+'：功能完善中 😊')} onTouchStart={()=>{}}
            style={{display:'flex',alignItems:'center',gap:12,width:'100%',background:'#fff',border:'1px solid #E5ECDE',borderRadius:14,padding:13,marginTop:8,fontSize:14,color:'#3A4437',textAlign:'left',touchAction:'manipulation'}}>
            <span style={{fontSize:18}}>{icon}</span><span style={{flex:1}}>{label}</span><span style={{color:'#A99585'}}>›</span>
          </button>
        ))}
      </div>

      <button onTouchStart={exportAll} onClick={exportAll} style={{marginTop:16,width:'100%',height:46,borderRadius:14,border:'1.5px solid #5B8C5A',background:'#fff',color:'#5B8C5A',fontSize:14,fontWeight:700,touchAction:'manipulation'}}>📤 导出备份</button>
      <div style={{textAlign:'center',color:'#A99585',fontSize:11,marginTop:14}}>水哥清单 v1.0.0</div>

      {editing && (
        <div style={{position:'fixed',inset:0,background:'rgba(60,40,30,.4)',zIndex:20,display:'flex',alignItems:'flex-end'}} onClick={()=>setEditing(false)}>
          <div style={{background:'#FFFDF9',width:'100%',borderRadius:'24px 24px 0 0',padding:22,paddingBottom:40}} onClick={e=>e.stopPropagation()}>
            ...
            <button onTouchStart={save} onClick={save} style={{marginTop:16,width:'100%',height:48,borderRadius:24,border:'none',background:'#5B8C5A',color:'#fff',fontSize:16,fontWeight:700,touchAction:'manipulation'}}>保存</button>
          </div>
        </div>
      )}
