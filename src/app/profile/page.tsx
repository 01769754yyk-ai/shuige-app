...
export default function Profile(){
  ...
  useEffect(()=>{ const n=localStorage.getItem(NAME_KEY); if(n) setName(n); },[]);
  ...
  const importFile=(file:File)=>{
    ...
    reader.onload=()=>{
      try{
        ...
        ALL_KEYS.forEach(k=>{
          if(data[k]!==undefined){ try{ localStorage.setItem(k, typeof data[k]==='string'?data[k]:JSON.stringify(data[k])); }catch{} }
        });
        setMsg('✅ 已恢复数据，请刷新页面查看');
  ...
  return (
    <div style={{minHeight:'100vh',padding:'16px 14px 80px',background:'#F4F7F0',color:'#3A4437',fontSize:15}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:12,background:'#fff',border:'1px solid #E5ECDE',borderRadius:14,padding:12,marginTop:12}}>
        <div style={{width:44,height:44,borderRadius:'50%',background:'#EAF4EA',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><span style={{fontSize:16,fontWeight:800}}>{'今日'}</span></div>
        <div style={{flex:1}}>
          <div style={{fontSize:13,fontWeight:700}}>水獭陪伴 · 慢慢来</div>
          <div style={{fontSize:11,color:'#8A9785',marginTop:2}}>「把喜欢的事慢慢做完」</div>
        </div>
      </div>
      ...
      <div style={{textAlign:'center',color:'#A99585',fontSize:11,marginTop:18}}>水哥清单 v1.1.0 · 支持备份恢复</div>
