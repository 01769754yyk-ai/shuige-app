  ...
  const send=async()=>{
    ...
    try{
      const res=await fetch('https://api.deepseek.com/chat/completions',{
        ...
        body:JSON.stringify({model:'deepseek-chat',messages:[{role:'system',content:'你是一个温柔可爱、简洁的助手，用中文回答'},...nowMsgs],stream:false})
      });
      ...
      setMsgs([...nowMsgs,{role:'assistant',content:'⚠️ 连接 AI 失败。可能原因：1) 浏览器跨域限制（CORS）2) key 无效或欠费。建议部署后测试，或改用服务器代理接 DeepSeek。'}]);
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{minHeight:'100vh',padding:'16px 14px 80px',background:'#F4F7F0',color:'#3A4437',fontSize:15,display:'flex',flexDirection:'column'}}>
      ...
      <div style={{display:'flex',gap:8,marginTop:12,position:'sticky',bottom:70,background:'#F4F7F0',padding:'8px 0'}}>
        <input value={input} onChange={e=>setInput(e.target.value)} placeholder="输入问题…" onKeyDown={e=>{if(e.key==='Enter')send()}}
          style={{flex:1,padding:12,borderRadius:20,border:'1.5px solid #E5ECDE',fontSize:14,background:'#fff',outline:'none'}}/>
        <button onTouchStart={send} onClick={send} disabled={loading} style={{padding:'12px 18px',borderRadius:20,border:'none',background:loading?'#A8B2A5':'#5B8C5A',color:'#fff',fontSize:14,fontWeight:700,touchAction:'manipulation'}}>发送</button>
      </div>
