...
export default function Diary(){
  ...
  return (
    <div style={{minHeight:'100vh',padding:'16px 14px 80px',background:'#F4F7F0',color:'#3A4437',fontSize:15}}>
      ...
      <button onTouchStart={()=>{setEditId(null);setShowAdd(true)}} onClick={()=>{setEditId(null);setShowAdd(true)}} style={{marginTop:14,width:'100%',height:48,borderRadius:14,border:'none',background:'#5B8C5A',color:'#fff',fontSize:15,fontWeight:700,touchAction:'manipulation'}}>＋ 写日记</button>
      <button onTouchStart={exportData} onClick={exportData} style={{marginTop:8,width:'100%',height:42,borderRadius:14,border:'1.5px solid #5B8C5A',background:'#fff',color:'#5B8C5A',fontSize:14,fontWeight:700,touchAction:'manipulation'}}>📤 导出备份</button>
        ...
        {entries.length? entries.map(e=>(
          <div key={e.id} style={{background:'#fff',border:'1px solid #E5ECDE',borderRadius:14,padding:12,marginTop:8}}>
            <div style={{display:'flex',alignItems:'center'}}>
              ...
              <button onClick={()=>setOpenId(openId===e.id?null:e.id)} style={{background:'none',border:'none',color:'#8A9785',fontSize:12}}>{openId===e.id?'收起':'展开'}</button>
            </div>
            ...
            {openId===e.id && (
              <div style={{display:'flex',gap:8,marginTop:10}}>
                <button onClick={()=>openEdit(e)} style={{flex:1,padding:'7px',borderRadius:10,border:'1px solid #5B8C5A',background:'#fff',color:'#5B8C5A',fontSize:13}}>编辑</button>
                <button onClick={()=>del(e.id)} style={{flex:1,padding:'7px',borderRadius:10,border:'1px solid #E07A5F',background:'#fff',color:'#E07A5F',fontSize:13}}>删除</button>
              </div>
            )}
          </div>
      ...
      {showAdd && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.4)',zIndex:20,display:'flex',alignItems:'flex-end'}} onClick={()=>setShowAdd(false)}>
          <div style={{background:'#FFFDF9',width:'100%',borderRadius:'24px 24px 0 0',padding:22,paddingBottom:40,maxHeight:'85vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:18,fontWeight:800,marginBottom:12}}>{editId?'编辑日记':'写日记'}</div>
            <div style={{display:'flex',gap:6,marginBottom:12}}>{moods.map(m=>(<button key={m} onClick={()=>setMood(m)} style={{fontSize:24,opacity:mood===m?1:.3,background:'none',border:'none'}}>{m}</button>))}</div>
            ...
            <button onTouchStart={saveEntry} onClick={saveEntry} style={{marginTop:14,width:'100%',height:48,borderRadius:24,border:'none',background:'#5B8C5A',color:'#fff',fontSize:16,fontWeight:700,touchAction:'manipulation'}}>{editId?'保存修改':'保存日记'}</button>
          </div>
        </div>
      )}
